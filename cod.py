...
for airport in airports:
    ...
    files += [(os.path.join(path_images, airport, f.replace('-01-01-01-01-01.txt', '_image.png')), os.path.join(path_annotations, airport, f)) for f in fs]
...
# --------------------------------------------------------------------------------------------- #
# Инициализация набора патчей изображения                                                                            #
# --------------------------------------------------------------------------------------------- #
def load_data():
    positives = []
    negatives = []
    with tqdm(total=len(files), file=sys.stdout) as pbar:
        ...
        for img_name, ann_name in files:
            # Загрузка аннотаций к самолетам
            ann_list = []
            ...
            # Обрезка образцов из исходного изображения
            size = 25
            step = 3
            for cc in ann_list:
                for x in range(-1, 2):
                    for y in range(-1, 2):
                        ...
                        if c[0] - size >= 0 and c[0] + size < img.shape[0] and c[1] - size >= 0 and c[1] + size < \
                                img.shape[1]:
            ...
            # Дополнительные отрицательные сэмплы, отобранные случайным образом по всему изображению
                ...
                if c[0] - size >= 0 and c[0] + size < img.shape[0] and c[1] - size >= 0 and c[1] + size < img.shape[1]:
                    ...
                    for cc in ann_list:
                        if abs(cc[0] - c[0]) <= size or abs(cc[1] - c[1]) <= size:
                            flag = False
                            break
                    # Отбросить, если точка выборки находится слишком близко к аннотированной точке или если она попадает в пустую область изображения
                    if flag and np.sum(img[c[0] - size:c[0] + size + 1, c[1] - size:c[1] + size + 1]) > 0:
                        negatives.append(img[c[0] - size:c[0] + size + 1, c[1] - size:c[1] + size + 1].copy())

            pbar.update(1)

            # Сохранение предела соотношения 1:2 между положительными и отрицательными выборками
...
# --------------------------------------------------------------------------------------------- #
# Обновление набора патчей образов и скорости обнаружения вычислительных ресурсов и частоты ложных обнаружений
def update_data(det_model, nms_model):
    ...
    with tqdm(total=len(files), file=sys.stdout) as pbar:
        ...
        for img_name, ann_name in files:
                ...
                for y in range(0, img.shape[0], block - 2 * radius):
                    ...
                    for x in range(0, img.shape[1], block - 2 * radius):
                        ...
                        if np.sum(img[y:y + block, x:x + block]) == 0:
                            continue
                        img_crop = torch.from_numpy(
                            np.transpose(img[y:y + block, x:x + block], (2, 0, 1))).float().unsqueeze(0)#.cuda()
                        mask_crop = det_model(img_crop)
                        full_mask[y + radius:min(y + block - radius, img.shape[0] - radius),
                        x + radius:min(x + block - radius, img.shape[1] - radius)] = mask_crop[0, :, :,
                                                                                     :].cpu().numpy().transpose(
                            (1, 2, 0))
            ...
            negatives += neg
            pbar.update(1)
        return total_correct / total_annotated, (total_dets - total_correct) / total_dets, np.asarray(negatives,
                                                                                                      dtype=np.float32)
...
def eval(det_model, imgs, labels, batch_size):
    ...
    with torch.no_grad():
        for i in range(0, len(imgs), batch_size):
            batch_data = torch.from_numpy(np.transpose(imgs[i:i + batch_size], (0, 3, 1, 2)))
            batch_target = torch.tensor(labels[i:i + batch_size], dtype=torch.long)
            batch_pred = det_model(batch_data)
            batch_pred = (batch_pred > 0.5).long()
            for t, p in zip(batch_target.view(-1), batch_pred.view(-1)):
                confusion_matrix[t, p] += 1
    det_model.train()
    return 100.0 * confusion_matrix[0, 0] / (confusion_matrix[0, 0] + confusion_matrix[0, 1]), 100.0 * \
...
def train(det_model, optimizer, imgs, labels, batch_size, iterations, epoch):
    ...
    # Инициализация списков для потерь и точности
    losses = []
    accuracies = []

    with tqdm(total=iterations, file=sys.stdout) as pbar:
        pbar.set_description(
            'Epoch #{} of training (POS {:.2f}% / NEG {:.2f}%)'.format(epoch + 1, pos_acc, neg_acc))
        det_model.train()
        for i in range(iterations):
            # случайный выбор batch_size изображений из обучающего набора
            batch = np.random.permutation(len(imgs))[:batch_size]
            # аугментация данных: горизонтальные и вертикальные перевороты, и повороты на 90 градусов
            ...
            # создание тензоров torch для изображений и меток текущей партии
            batch_data = torch.from_numpy(np.transpose(np_batch_data, (0, 3, 1, 2)))
            batch_target = torch.tensor(labels.take(batch, axis=0), dtype=torch.long).float()
            # выполнение одной оптимизационной итерации
            det_model.zero_grad()
            ...
            # проверка производительности каждые 100 итераций
            if i % 100 == 99:
                neg_acc, pos_acc = eval(det_model, imgs, labels, batch_size)
                accuracies.append((pos_acc, neg_acc))  # Добавление точности в список
                pbar.set_description(
                    'Epoch #{} of training (POS {:.2f}% / NEG {:.2f}%)'.format(epoch + 1, pos_acc,
                                                                               neg_acc))
            pbar.update(1)

        return losses, accuracies  # Возврат списков потерь и точности
# После завершения всех эпох обучения
...
print("Training data loaded: {} positive and {} negative samples\n".format(len(train_pos),
                                                                           len(train_neg)))

# run 1st epoch of training
losses, accuracies = train(det_model, optimizer, train_imgs, train_labels, batch_size, num_iter,
                           0)
...
for epoch in range(1, max_epochs):
    if len(false_positives) > 0:
        ...
        if len(train_neg) >= len(train_pos) and len(false_positives) >= len(train_pos):
            batch1 = train_neg.take(np.random.permutation(len(train_neg))[:len(train_pos)],
                                    axis=0)
            batch2 = false_positives.take(
                np.random.permutation(len(false_positives))[:len(train_pos)], axis=0)
        elif len(train_neg) >= len(train_pos):
            batch1 = train_neg.take(np.random.permutation(len(train_neg))[
                                    :2 * len(train_pos) - len(false_positives)], axis=0)
        elif len(false_positives) >= len(train_pos):
            batch2 = false_positives.take(np.random.permutation(len(false_positives))[
                                          :2 * len(train_pos) - len(train_neg)], axis=0)
        ...
        # run training epoch
        losses, accuracies = train(det_model, optimizer, train_imgs, train_labels, batch_size,
                                   num_iter, epoch)
